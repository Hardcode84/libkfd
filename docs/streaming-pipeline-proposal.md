...
Status: design proposal. Not implemented. The `quake_raster` library currently
ships the cudaraster-shaped pipeline described in
`quake-compute-rasterizer-plan.md` (separate dispatches per stage, host-driven
fence between stages, worst-case-sized intermediates). This document
describes a candidate replacement: a persistent-megakernel streaming pipeline
fed by a shared host↔GPU ring buffer, with dynamic role selection across
workgroups.
...
- Worst-case-sized intermediates. `tile_indices`, `tile_counts`,
  `tile_overflows`, `tile_depth_min/max` are all sized for the maximum
  primitive count the renderer must handle.
...
For Quake-shaped scenes the round-robin scan is likely sufficient.
...
### 3.1 Current `quake_raster`

| Concern | Current | Streaming proposal |
|---|---|---|
| Pipeline stage overlap | None (barrier-per-stage) | Full (megakernel) |
| Intermediate sizing | Worst-case `tile_indices[tile][256]` | Bounded queues, backpressure |
| Host overlap with GPU | None within frame | Full (host pushes during dispatch) |
| Per-frame dispatch count | 4 | 1 |
| Bin-counter atomics | Per-tile `tile_counts`/`tile_overflows` | None (wave-disjoint single producer) |
...
| Implementation cost | Shipped | Significant |
...
For Quake on AMDGPU, cuRE-style is probably the better trade. The host's
per-frame work is small; primitive counts fit in <1MB; the latency overlap
gain is marginal. The streaming proposal earns its complexity when the host
pipeline is heavy (skinning, animation, complex culling, LOD).
...
- Scene complexity rises beyond Quake-shaped (denser triangles, higher
...
1. **Compile wave64 across the board**. Wastes lanes on RDNA's
   ...
   archs. Simplest.
2. **Per-arch `W` tuning**. `quake_raster_add_gpu_kernel` already
   cross-compiles per arch; `W` and the fine-tile geometry can be
   per-arch macros.
...
## 7. Where This Slots In The Existing Roadmap

`quake-compute-rasterizer-todo.md` lists Stage F (hierarchical binning) as a
follow-on. Stage F as currently described is "add a coarse level above the
existing 16×16 fine tiles, still single dispatch, still barrier-per-stage".
The streaming proposal is a more invasive alternative to Stage F that
collapses Stage F's two-level binning into the megakernel and removes the
inter-stage barriers entirely.

The two are not stackable. Choosing the streaming proposal subsumes the
Stage F redesign.

A reasonable staging:

1. Land Stage F as currently described (additive, less risky).
2. Profile to determine whether the cost saved by streaming is worth the
   engineering. For Quake specifically, likely not. For a generalized
   compute-rasterizer beyond Quake, likely yes.
3. If yes, implement the hybrid (bulk upload + on-GPU streaming, §3.3) as a
   first step. Adds the megakernel and on-GPU queues without the
   host↔GPU PCIe-atomic complexity.
4. If full streaming is justified, extend with the host↔GPU ring.

## 8. Reading

- `cure-streaming-queues.md` — primitive vocabulary used throughout.
- `quake-compute-rasterizer-prior-art.md` — lineage and why cuRE-shaped
  designs are the right reference for this workload.
- `quake-compute-rasterizer-plan.md` — current architecture being replaced.
- `quake-compute-rasterizer-todo.md` — Stage F as currently scoped.
